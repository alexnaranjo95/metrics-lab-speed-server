...
export async function runOptimizationAgent(siteId: string): Promise<AgentReport> {
  ...
  const state: AgentState = {
    siteId, runId, phase: 'analyzing', iteration: 0, maxIterations: MAX_ITERATIONS,
  ...
  const log = (msg: string) => {
    ...
    // Also emit to any connected SSE/WebSocket clients
  ...
  try {
    ...
    log(`  Site: ${site.siteUrl}`);
    log(`  Model: Claude Opus 4 (claude-opus-4-20250514)`);
    ...
    for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
      ...
      // Log what settings are being applied
      ...
      if (buildResult.status === 'failed') {
        ...
        // Try to recover with safer settings
    ...
    // Keep agent state for 1 hour for status queries
...
function logSettingsDecisions(settings: Record<string, any>, log: (msg: string) => void) {
  const s = settings;
  if (s.images) {
    log(`[settings] Images: enabled=${s.images.enabled}, WebP quality=${s.images.webp?.quality || 80}, AVIF=${s.images.convertToAvif ? 'ON' : 'OFF'}`);
  }
  if (s.css) {
    log(`[settings] CSS: purge=${s.css.purge ? 'ON' : 'OFF'}, aggressiveness=${s.css.purgeAggressiveness || 'safe'}, critical=${s.css.critical ? 'ON' : 'OFF'}`);
  }
  if (s.js) {
    log(`[settings] JS: removeJquery=${s.js.removeJquery ? 'YES (risky!)' : 'NO (safe)'}, emoji=${s.js.removeScripts?.wpEmoji ? 'remove' : 'keep'}`);
  }
  if (s.fonts) {
    log(`[settings] Fonts: selfHost=${s.fonts.selfHostGoogleFonts ? 'ON' : 'OFF'}, display=${s.fonts.fontDisplay || 'swap'}`);
  }
  if (s.video) {
    log(`[settings] Video: facades=${s.video.facadesEnabled ? 'ON' : 'OFF'}`);
  }
}
